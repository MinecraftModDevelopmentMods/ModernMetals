...
public class EntityHorseTransformer implements ITransformer {
    ...
    public void transform(ClassNode node, boolean dev) {
        for (MethodNode methodNode : node.methods) {
            if (methodNode.name.equals(dev ? "entityInit" : "")) {
            ...
            } else if (methodNode.name.equals(dev ? "setHorseArmorStack" : "")) {
                InsnList inject = new InsnList();
                inject.add(new VarInsnNode(ALOAD, 0));
            ...
            } else if (methodNode.name.equals(dev ? "setHorseTexturePaths" : "")) {
                for (AbstractInsnNode insnNode : methodNode.instructions.toArray()) {
                    if (insnNode instanceof MethodInsnNode && ((MethodInsnNode) insnNode).name.equals(dev ? "getTextureName" : "")) {
                        InsnList inject = new InsnList();
                        inject.add(new VarInsnNode(ALOAD, 0));
