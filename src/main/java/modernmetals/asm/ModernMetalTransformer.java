...
public class ModernMetalTransformer implements IClassTransformer {
    @Override
    public byte[] transform(String name, String transformedName, byte[] bytes) {
        for (ITransformer transformer : ModernMetalPlugin.transformerList) {
            if (transformedName.equals(transformer.getTarget())) {
                ClassReader classReader = new ClassReader(bytes);
                ClassNode classNode = new ClassNode();
                classReader.accept(classNode, 0);
                transformer.transform(classNode, ModernMetalPlugin.inDevelopment);
